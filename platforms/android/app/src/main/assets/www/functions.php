...
function get_my_db()
{
    static $db;
    if (!$db) {
					$db = new mysqli("localhost", "root", "inet@123", "lsfdb");
    }
    return $db;
}
...
// FUNCTIONS OF LSF START

function get_tutorials($id=0){

	$db=get_my_db();
	
	$data=array();
	
	$query="select * from inet_tutorial where status='Y' ";
	...
	$sql=$db->query($query) or die("error in query load TUORIAL---->".$db->error);
	
	$data=fetchmysqlDB($sql);
	return $data;
}

function get_voicedata($vid=0){

	$db=get_my_db();
	
	$data=array();
	
	$query="select * from inet_ownvoice where Status='Y' ";
	...
	$sql=$db->query($query) or die("error in query load ownvoice---->".$db->error);
	
	$data=fetchmysqlDB($sql);
	return $data;
}

function get_selfprac($tid=0){

	$db=get_my_db();
	
	$data=array();
	
	$query="select * from inet_self_practice_qusupl	 where status='Y' ";
	...
	$sql=$db->query($query) or die("error in query load selfvoice---->".$db->error);
	
	$data=fetchmysqlDB($sql);
	return $data;
}

function get_videopage($tid=0){

	$db=get_my_db();
	
	$data=array();
	
	$query="select * from inet_listning_video	 where status='Y' ";
	
	$query .= " ORDER BY today_date";
